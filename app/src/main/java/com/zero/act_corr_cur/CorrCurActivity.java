...
public class CorrCurActivity extends AppCompatActivity {

    RecyclerView recyclerView;
    CorrCurAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_corr_cur);
        getSupportActionBar().hide();

        //Iniciar ProgressBar
        final ProgressBar progressBar = (ProgressBar) findViewById(R.id.corr_cur_spin);
        Sprite sprite = new DoubleBounce();
        progressBar.setIndeterminateDrawable(sprite);

        //Search
        searchInput = findViewById(R.id.corr_cur_search_input);
        ...
        recyclerView = findViewById(R.id.corr_cur__rv);
        ...
        mAPIService.GetCorrelatividadCursado(token.toString(), estudiante.get_id()).enqueue(new Callback<ResponseGetCorrelatividadCursado>() {
            @Override
            public void onResponse(Call<ResponseGetCorrelatividadCursado> call, Response<ResponseGetCorrelatividadCursado> response) {
                if (response.isSuccessful()) {
                    Gson objetoConsola = new Gson();
                    List<CorrelatividadCursado> mdata = new ArrayList<CorrelatividadCursado>();
                    Log.i("Prueba Rest", objetoConsola.toJson(response.body()));
                    for (CorrelatividadCursado objeto : response.body().getCorrelatividadcursado()) {
                        int img = ConverterImg(objeto.getAnio());
                        mdata.add(new CorrelatividadCursado(objeto.get_id(),objeto.getMateria(),objeto.getPlan(),img,
                                objeto.getIdEstudiante(),
                                objeto.getCorrelatividad()));
                        Log.i("Prueba Rest", objetoConsola.toJson(objeto.getMateria()).toString());
                    }
                    adapter = new CorrCurAdapter(getApplicationContext(), mdata);
            ...
            @Override
            public void onFailure(Call<ResponseGetCorrelatividadCursado> call, Throwable t) {
                progressBar.setVisibility(View.INVISIBLE);
                FragmentManager fragmentManager = getSupportFragmentManager();
                Dialog dialog =Dialog.newInstance("api");;
                dialog.show(fragmentManager, "Alert");
